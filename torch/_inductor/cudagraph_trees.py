...
def extract_reinterpret_tensor_info(call_string):
    # Regex pattern to match the reinterpret_tensor call
    pattern = r'reinterpret_tensor\((\w+), \(([^)]+)\), \(([^)]+)\), ([^)]+)\)'
    match = re.search(pattern, call_string)
    
    if match:
        tensor_name = match.group(1)  # First argument (Tensor name)
        
        # Try to process size tuple
        try:
            size_tuple = tuple(int(x.strip()) for x in match.group(2).split(',') if x.strip().isdigit())
        except ValueError:
            return tensor_name, None, None, None
        
        # Try to process stride tuple
        try:
            stride_tuple = tuple(int(x.strip()) for x in match.group(3).split(',') if x.strip().isdigit())
        except ValueError:
            return tensor_name, None, None, None
        
        # Try to process offset increment
        try:
            offset_increment = int(match.group(4).strip())
        except ValueError:
            return tensor_name, None, None, None
        
        # If all parsing succeeded, return the extracted values
        return tensor_name, size_tuple, stride_tuple, offset_increment
    else:
        print(f"{call_string} does not match the pattern")
        return None

def reconstruct_reinterpret_tensor_arguments(original_tensor, reinterpreted_tensor):
    recovered_size = reinterpreted_tensor.size()
    recovered_stride = reinterpreted_tensor.stride()
    recovered_offset = reinterpreted_tensor.storage_offset() - original_tensor.storage_offset()
    return recovered_size, recovered_stride, recovered_offset
...
########################################################################################
#from torch._inductor.compile_fx import get_expanded_dims, index_expanded_dims
# the above import was giving issue
...
############################### checkpoint to and restore from disk ###############################
import tempfile
import os

def checkpoint_inputs_to_disk(inputs):
    inputs_reference = inputs.copy()
    ...
    for i, t in enumerate(inputs):
        ...
        if isinstance(t, torch.Tensor):
            temp_file_path = os.path.join(temp_dir.name, f"tensor_{i}.pt")
            torch.save(t, temp_file_path)
            temp_files.append(temp_file_path)
        else:
            temp_files.append(t)
    
    return inputs_reference, temp_files, temp_dir

def restore_inputs_from_disk(inputs_reference, temp_files, temp_dir, cleanup_temp_dir=False):
    inputs = inputs_reference.copy()

    for i in range(len(inputs)):
        if isinstance(temp_files[i], str):  # Check if it's a path to a saved tensor
            restored_tensor = torch.load(temp_files[i])
            if isinstance(inputs[i], (torch.nn.Parameter, torch.Tensor)):
                # get the expanded dims
                expanded_dims = get_expanded_dims(inputs[i])
                index_expanded_dims(inputs[i], expanded_dims).data.copy_(index_expanded_dims(restored_tensor, expanded_dims).data)
            del restored_tensor
        else:
            inputs[i] = temp_files[i]
...
def cudagraph_selective_disable_profile_debug_enabled():
    return os.environ.get(
        "TORCHINDUCTOR_CUDAGRAPH_SELECTIVE_DISABLE_PROFILE_DEBUG", "1"
    ) == "1"

class function_obj:
    def __init__(self):
        self.fn = None 
        self.indirect_fn = None
        self.profiled = None
        self.is_cudagraph_disabled_by_profile = None
        self.cpu_tensor_indices = []

# store the time of the start of the cudagraph replay for printing the replay time
replay_start_time = None
replay_end_time = None

def cudagraphify_impl(model, inputs, static_input_idxs, *args, **kwargs):
    fn_cache: Dict[Tuple[int, ...], function_obj] = {}
    ...
    # Access is_backward and is_inference from kwargs
    is_backward = kwargs.get('is_backward', None)
    is_inference = kwargs.get('is_inference', None)
    is_forward = True if (not is_backward and not is_inference) else False
    device_index = kwargs.get('device_index', None)

    un_cudagraphified_callable = model
    # explicitly doing input alignment because
    # cudagraphs does its own aligning of inputs
    new_callable = align_inputs(
        un_cudagraphified_callable, inputs, static_input_idxs
    )
    if new_callable is not un_cudagraphified_callable:
        un_cudagraphified_callable = new_callable
                    
    del inputs

    def cudagraph_tree_manager_pending_recording_epilogue():
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        if manager is not None:
            return manager.pending_recording_epilogue == True
        return False
    ...
    def stash_manager_direct_node():
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        stash_node(manager.direct_node)
        manager.direct_node = None
    
    def stash_manager_indirect_node():
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        stash_node(manager.indirect_node)
        manager.indirect_node = None
        
    def stash_cudagraphified_callables(): # this function would require a change based on the new implementation
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        manager.pending_recording_epilogue = False
        # node_to_stash = [manager.direct_node, manager.indirect_node]
        # nodes = []
        # nodes.extend(node_to_stash)
        # while nodes:
        #     node = nodes.pop()
        #     node.remove_node_cached_tensors()
        #     node.graph = None
        stash_manager_direct_node()
        stash_manager_indirect_node()

    def CUDAGraphNode_run_during_profile(inputs): # this function would require a change based on the new implementation
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        assert manager.direct_node is not None
        node = manager.direct_node 
        
        if node.check_invariants(inputs):
            return node.profile_run(inputs)
        else:
            raise Exception("Invariants violated during direct profile run")
    def CUDAGraphNodeIndirect_run_during_profile(inputs): # this function would require a change based on the new implementation
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        assert manager.indirect_node is not None
        node = manager.indirect_node
        
        if node.check_invariants(inputs):
            return node.profile_run(inputs)
        else:
            raise Exception("Invariants violated during indirect profile run")
        
    def cudagraph_tree_manager_set_running_forwards_with_pending_backwards(value):
        manager = get_manager(
            device_index, create_if_none_exists=False
        )
        manager.running_forwards_with_pending_backwards=value

    def cudagraph_tree_manager_early_exit_due_to_mutation():
        manager = torch._inductor.cudagraph_trees.get_manager(
            device_index, create_if_none_exists=False
        )
        return manager.early_exit_due_to_input_mutation
    
    def cudagraph_tree_manager_early_exit_due_to_mutation_reset():
        manager = torch._inductor.cudagraph_trees.get_manager(
            device_index, create_if_none_exists=False
        )
        manager.early_exit_due_to_input_mutation = False

    def tensors_on_cpu(fn_obj: function_obj, list):
        for i, p in enumerate(list):
            try: # not all p might be tensors
                if 'cuda' not in str(p.device):
                    fn_obj.cpu_tensor_indices.append(i)
            except:
                pass
    ...
    def deferred_cudagraphify(inputs):
        ...
        if fn is not None:
            if not fn.profiled:
                inputs_reference, temp_files, temp_dir = checkpoint_inputs_to_disk(inputs)
                torch.cuda.synchronize()
                # Save the current state of the random generators
                cpu_rng_state = torch.get_rng_state()
                gpu_rng_state = torch.cuda.get_rng_state() if torch.cuda.is_available() else None

            if fn.is_cudagraph_disabled_by_profile and not not fn.cpu_tensor_indices:
                for idx in fn.cpu_tensor_indices:
                    inputs[idx].data = inputs[idx].data.cpu()
            ...
            if fn.is_cudagraph_disabled_by_profile and not is_inference:
                # # if we are in the bypassed forward, then we set the pending backward flag to True
                # if self.is_forward:
                #     self.cudagraph_tree_manager_set_running_forwards_with_pending_backwards(True)
                # if we are in the bypassed backward, then we set the pending backward flag to False
                if is_backward:
                    cudagraph_tree_manager_set_running_forwards_with_pending_backwards(False)
            
            if not fn.profiled and cudagraph_tree_manager_early_exit_due_to_mutation():
                fn.profiled = True
                cudagraph_tree_manager_early_exit_due_to_mutation_reset()
                
            if not fn.profiled and cudagraph_tree_manager_pending_recording_epilogue():
                indirect_cudagraphified_ret, direct_cudagraphified_ret = ret
        
                inputs = restore_inputs_from_disk(inputs_reference, temp_files, temp_dir)

                for i in range(len(inputs)):
                ##############################################################
                # For certain triton kernels (in applications like ElectraForCausalLM) there is a call to `aten._scaled_dot_product_flash_attention_backward.default`
                # The way it works is as follows:
                # when not capturing a CUDAGraph, it internally https://github.com/pytorch/pytorch/blob/d44ab8ba6de5821754ce2bc5fd3d97d294468b1a/aten/src/ATen/native/transformers/cuda/flash_attn/flash_api.cpp#L940C4-L951C38
                # deferences the data pointer of the input tensor, but while capturing a CUDAGraph, it does not dereference the data pointer of the input tensor.
                # The implicit assumption here is that, during "non CUDAGraph" execution, you are passing in a CPU tensor, or else, if you pass in a GPU tensor, the dereferencing of the data pointer
                # shall cause segmentation faulting on the device side, but unfortunately, during recording and hence forward, all the tensors are on the GPU, in the input.
                # So, a hacky way out is, when this CompileFxGraph module is running the warmup, it is provided with the `inputs` with the original state, with tensors on CPU.
                # We get that information about CPU tensors during warmup, and while trying to profile the un_cudagraphified_callable, we copy the values from the GPU to CPU as follows.
                ##############################################################
                    if i in fn.cpu_tensor_indices:
                ...
                torch.cuda.nvtx.range_push("un_cudagraphified_callable")
                time1 = timer()
                un_cudagraphied_ret = un_cudagraphified_callable(inputs)
                torch.cuda.synchronize()
                time2 = timer()
                torch.cuda.nvtx.range_pop()
                un_cudagraphified_time = time2 - time1
                ...
                torch.cuda.nvtx.range_push("indirect cudagraphified_callable")
                time1 = timer()
                CUDAGraphNodeIndirect_run_during_profile(inputs)#self.current_callable(inputs) 
                torch.cuda.synchronize()
                time2 = timer()
                torch.cuda.nvtx.range_pop()

                indirect_cudagraphified_time = time2 - time1
                

                inputs = restore_inputs_from_disk(inputs_reference, temp_files, temp_dir, cleanup_temp_dir=True)
                torch.cuda.synchronize()
                # Restore the state of the random generators
                torch.set_rng_state(cpu_rng_state)
                if gpu_rng_state is not None:
                ...
                torch.cuda.nvtx.range_push("direct cudagraphified_callable")
                time1 = timer()
                CUDAGraphNode_run_during_profile(inputs)#self.current_callable(inputs) 
                # torch.cuda.synchronize()
                time2 = timer()
                torch.cuda.nvtx.range_pop()

                direct_cudagraphified_time = time2 - time1
                cudagraph_replay_time = replay_end_time - replay_start_time#time2 - replay_start_time
                
                if cudagraph_selective_disable_profile_debug_enabled():
                    print(  f"un_cudagraphified_time: {un_cudagraphified_time}, "
                            f"direct_cudagraphified_time: {direct_cudagraphified_time}, "
                            f"indirect_cudagraphified_time: {indirect_cudagraphified_time}, "
                            f"cudagraph_replay_time: {cudagraph_replay_time}")
                min_time = min(un_cudagraphified_time, direct_cudagraphified_time, indirect_cudagraphified_time)
                
                if min_time == un_cudagraphified_time:
                    if cudagraph_selective_disable_profile_debug_enabled():
                        print("un_cudagraphified_callable is fastest, we are disabling cudagraph for this fx graph module")
                    stash_cudagraphified_callables()
                    #stashing it first before setting the `fn.fn` is important or else the manager shall be garbage collected
                    # by _finalize_tree_manager when the reference to the `run` returned by cudagraphify is lost.
                    fn.fn = un_cudagraphified_callable
                    fn.is_cudagraph_disabled_by_profile = True
                    ret = un_cudagraphied_ret
                elif min_time == direct_cudagraphified_time:
                    if cudagraph_selective_disable_profile_debug_enabled():
                        print("direct cudagraphified_callable is fastest, "
                              "we are choosing the direct version of cudagraph for this fx graph module")
                    manager = get_manager(
                                device_index, create_if_none_exists=False
                            )
                    manager.record_function_epilogue(manager.direct_node)
                    if manager.applied_checkpoint_awaiting_recording:
                        manager.applied_checkpoint_awaiting_recording = False
                    ret = direct_cudagraphified_ret
                    stash_manager_indirect_node()
                    manager.direct_node = None
                elif min_time == indirect_cudagraphified_time:
                    if cudagraph_selective_disable_profile_debug_enabled():
                        print("indirect cudagraphified_callable is fastest, "
                              "we are choosing the indirect version of cudagraph for this fx graph module")
                    manager = get_manager(
                                device_index, create_if_none_exists=False
                            )
                    manager.record_function_epilogue(manager.indirect_node)
                    if manager.applied_checkpoint_awaiting_recording:
                        manager.applied_checkpoint_awaiting_recording = False
                    ret = indirect_cudagraphified_ret
                    stash_manager_direct_node()
                    manager.indirect_node = None
                else:
                    raise Exception("Something went wrong, cudagraphify_impl...")
                
                fn.profiled = True
                
                
                inputs_reference.clear()
                del inputs_reference
            return ret
        ...
        fn, out, indirect_fn = cudagraphify(model, inputs, new_static_input_idxs, *args, **kwargs)
        ...
        fn_obj.fn = fn; fn_obj.indirect_fn = indirect_fn; fn = fn_obj
        fn.profiled = False
        fn_cache[int_key] = fn
        return out
        ...
        with torch.cuda.device(
            ...
            for output_index, (input_index, size_tuple, stride_tuple, offset) \
                ...
                if size_tuple is None:
                    size_tuple, stride_tuple, offset = reconstruct_reinterpret_tensor_arguments(new_inputs[input_index], 
                                                                                                        out[output_index])
...
class CUDAGraphNode:
    ...
    def __init__(
        ...
        if self.wrapped_function.is_backward and \
            ((self.parent is None and self.function_id.id!=0) or \
            (self.parent is not None and self.parent.function_id.id != self.function_id.id - 1)):
            self.static_input_idxs: List[int] = self.cudagraph_managed_idxs
        else:    
        ...
        ################################################# issue with managed static inputs#######################################
        ## when we hit a CUDAGraphNode init after a bypassed CUDAGraphNode, the cudagraph_managed_idxs are null 
        ## hence the non_managed_static_input_idxs == static_input_idxs. As a result the variable tensors (which are in the 
        ## should have been in the cudagraph_managed_idxs if we followed the non bypassed path) are being considered as 
        ## non managed idxs causing issue. 
        ## For the time being switch off this stability check of static input idxs
        #########################################################################################################################  
        self.non_managed_static_input_idxs = [] # switching the check off for now
        ####################################################################################################################
        ...
        if self.parent is not None and self.parent.function_id.id == self.function_id.id - 1:
        ...
        # there are situations, BertForQuestionAnswering, LayoutLMForMaskedLM, where the un_cudagraphified compiledFx
        # graph returns certain CPU tensors. Now those CPU tensors cannot be "reconstructed" from meta data, because
        # the api which PyTorch guys use can only create a GPU tensor. So, we push the CPU tensor on to the GPU.
        # NOTE: As of now, I am not sure of whether this simple push of CPU tensor to the GPU, and using that in the CUDAGraph,
        # shall have some use after deallocation issue or not, because that tensor (pushed GPU tensor) is not allocated on the
        # CUDAGraph memory pool.
        # But doing something as:
        # if inputs[i].device.type == "cpu":
        ...
        # causes check_memory_pool to fail, because the tensor is not accounted for as an output of CUDAGraph trees.
    ...
    def check_static_inputs_are_stable(self, new_inputs):
        ...
        if not torch._C._tensors_data_ptrs_at_indices_equal(
            ...
            #     )
            for idx, (t, data_ptr) in enumerate(zip(static_tensors, data_ptrs)):
                torch._check(
                    t.data_ptr() == data_ptr,
                    lambda: f"static input data pointer changed from {data_ptr} to {t.data_ptr()} at index {self.non_managed_static_input_idxs[idx]}"
                )
    ...
    ##################################################################################
    # added by me
    def profile_run(self, new_inputs):
        #torch.cuda.nvtx.range_push("CUDAGraphNode::run")
        try: # torchbench detectron2_maskrcnn_r_50_fpn inference
        ...
        global replay_end_time
        replay_start_time = timer()
        self.run_graph()
        torch.cuda.synchronize()
        replay_end_time = timer()
        # outputs = self.profile_reconstruct_outputs()
...
class CUDAGraphNodeIndirect:
    ...
    def run(self, new_inputs):
        ...
        # wrapped_function_static_input_idx_data_ptrs = [
        #     new_inputs[i].data_ptr()
        #     for i in self.wrapped_function.static_input_idxs
        #     if isinstance(new_inputs[i], torch.Tensor)
        # ]
        # print(f"CUDAGraphNodeIndirect::run::wrapped_function_static_input_idx_data_ptrs: {wrapped_function_static_input_idx_data_ptrs}")
        
        self._copy_inputs_and_remove_from_src(self.reconstructed_inputs, new_inputs)
        # new_inputs.clear()
        
        # print(f">>>> run:: start of the loop in node {self}")
        # print(f"{self.flat_ptr_tensor=}")
        # print(f"{self.indirection_args_indices=}")
        # print(f"{len(self.reconstructed_inputs)=}")
        # for i, t in enumerate(self.reconstructed_inputs):
        #     if i in self.indirection_args_indices:
        #         print(f"indirection arg {i}: {self.reconstructed_inputs[i]}, ", end="")
        #         print(f"{self.reconstructed_inputs[i].data_ptr()=} ", end="")
        #         if new_inputs[i] is not None:
        #             print(f"{new_inputs[i].data_ptr()=}, {new_inputs[i].device=}, {new_inputs[i]=}")
        #         else:
        #             print()
        #     elif isinstance(t, torch.Tensor):
        #         print(f"input {i}: {t.device=}, {t.data_ptr()=}")
        # print(f"<<<< run:: end of the loop in node {self}")    
    ...
    def _allocate_and_copy_recording_inputs(
        ...
        with warnings.catch_warnings(record=True), torch.cuda.device(
            ...
            #     if i in self.indirection_args_indices:
            #         print(f"indirection arg {i}: {recording_inputs[i].data_ptr()=}; {recording_inputs[i]=}; ", end=" ")
            #         print(f"{inputs[i].data_ptr()=}; {inputs[i].shape=}")
            #         if inputs[i].shape == torch.Size([1]):
            #             print(f"{inputs[i]=}")
...
class CUDAGraphTreeManager:
    ...
    def __init__(self, device_index: int):
        ...
        # added by me
        self.pending_recording_epilogue = False
        self.early_exit_due_to_input_mutation = False
        self.applied_checkpoint_awaiting_recording = False
    ...
    def _get_node_id(self) -> Optional[GraphID]:
        if self.current_node is None:
            return None
        elif isinstance(self.current_node, (CUDAGraphNode, CUDAGraphNodeIndirect, CUDAWarmupNode)):
    ...
    def _run(self, new_inputs: List[Tensor], function_id: FunctionID, indirection: bool= False):
        ...
        if self.non_cudagraph_managed_mutation_hint[node_id][function_id]:
            self.early_exit_due_to_input_mutation = True
    ...
    def record_function(self, new_inputs, function_id, indirection: bool = False) -> List[Optional[Tensor]]:
        ...
        self.indirect_node = CUDAGraphNodeIndirect(
                self.ids_to_funcs[function_id],
                graph_id,
                self.current_node,
                new_inputs,
        ...
        new_inputs = new_inputs_copy.copy() # we need to do new_inputs_copy.copy(), and not new_inputs_copy, because
                                            # CUDAGraphNode clears new_inputs and hence we shall lose the 
                                            # original data for the indirect first run.
        
        self.direct_node = CUDAGraphNode(
                self.ids_to_funcs[function_id],
                graph_id,
                self.current_node,
                new_inputs,
        ...
        self.pending_recording_epilogue = True # required for profiling to let this know
                                               # that the recording is done
        self.update_generation() # should I keep it here???? or in the epilogue?
        torch.cuda.synchronize()
        #return node.run_first_inputs(new_inputs)
        indirect_output = self.indirect_node.run_first_inputs(new_inputs_copy)
        direct_output = self.direct_node.run_first_inputs(new_inputs)
        return indirect_output, direct_output

    def record_function_epilogue(self, node: Union[CUDAGraphNode, CUDAGraphNodeIndirect]) -> None:
        if self.current_node is None:
            self.roots[node.function_id].append(node)
        else:
            self.current_node.add_child(node.function_id, node)
        self.current_node = node
        self.path_state = ExecutionState.RECORDING
        self.pending_recording_epilogue = False
    ...
    def run_eager(self, new_inputs, function_id: FunctionID):
        ...
        indirect_node = CUDAWarmupNodeIndirect(
            self.ids_to_funcs[function_id],
            self.current_node,
            self.cuda_graphs_thread_pool,
            self.graph,
    ...
    def add_function(
        ...
        return fn, fn(inputs), fn_indirection
    ...
    def apply_checkpoint_execution_state_in_allocator(self):
        ...
        """
        if self.applied_checkpoint_awaiting_recording:
            log.debug("Skipping checkpointing; applied checkpoint awaiting recording")
            return
        ...
        self.applied_checkpoint_awaiting_recording = True
